...
public class UserController {
	...
	@PutMapping("/softDeleteUser/{uid}")
	public ResponseEntity<?> softDeleteById(@PathVariable("uid") int uid) {
		User u = userService.getUserById(uid);
		if (u != null) {
			u.setDeleted(true);
			User savedEmp = userService.softDeleteUser(u);
			return new ResponseEntity<User>(savedEmp, HttpStatus.CREATED);
		} else {
	...
	public ResponseEntity<List<User>> getUserByName(@PathVariable("uname") String username) {

		return new ResponseEntity(userService.findUserByUname(username), HttpStatus.OK);
	}
